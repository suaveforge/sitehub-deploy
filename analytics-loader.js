(()=>{
  ...
  const API=(script?.dataset?.sitehubApi||'https://api-sitehub.suaveforge.com').replace(/\/$/,'')+'/api/v1/analytics/events';
  ...
  if(!first?.landingPage){
    first={landingPage:location.pathname+location.search,referrer:document.referrer||'',utmSource:params.get('utm_source')||'',utmMedium:params.get('utm_medium')||'',utmCampaign:params.get('utm_campaign')||''};
  ...
  const base=()=>({projectId,visitorId,sessionId,...first,deviceType:deviceType(),browser:browser(),language:(navigator.language||'').slice(0,80),timezone:(Intl.DateTimeFormat().resolvedOptions().timeZone||'').slice(0,120),screenWidth:screen?.width||0,screenHeight:screen?.height||0});

  const removeFromQueue=id=>writeQueue(readQueue().filter(x=>x.id!==id));
  const markAck=id=>{rememberAck(id);removeFromQueue(id)};
  const post=async item=>{try{const r=await fetch(API,{method:'POST',headers:{'Content-Type':'text/plain;charset=UTF-8'},credentials:'omit',keepalive:true,cache:'no-store',body:item.body});if(r.ok){markAck(item.id);return true}}catch{}return false;};
  const flush=async()=>{if(flushing)return;flushing=true;try{const acks=ackedIds();const queue=readQueue().filter(x=>!acks.has(x.id));for(const item of queue){item.attempts=Number(item.attempts||0)+1;writeQueue(queue);if(!await post(item))break}}finally{flushing=false}};
  const beaconPending=()=>{if(!navigator.sendBeacon)return;const acks=ackedIds();for(const item of readQueue()){if(acks.has(item.id))continue;try{navigator.sendBeacon(API,new Blob([item.body],{type:'text/plain;charset=UTF-8'}))}catch{}}};
  ...
  const capture=()=>{
    ...
    const payload={...base(),eventType:'page_view',hostname:location.hostname,pathname:location.pathname,pageTitle:document.title||'',referrer:first.referrer||'',metadata:{pageViewId,url:location.pathname+location.search,captureVersion:'20260908-sitehub-01',capturedAt:new Date().toISOString()}};
  ...
  const wrapHistory=name=>{const original=history[name];if(typeof original!=='function')return;history[name]=function(...args){const out=original.apply(this,args);routeCapture();return out};};
  wrapHistory('pushState');wrapHistory('replaceState');
  addEventListener('popstate',routeCapture,{passive:true});addEventListener('hashchange',routeCapture,{passive:true});addEventListener('online',()=>void flush(),{passive:true});addEventListener('pagehide',beaconPending,{capture:true});
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')beaconPending();else void flush()},{capture:true});
  capture();setTimeout(()=>void flush(),1200);setTimeout(()=>void flush(),4500);
